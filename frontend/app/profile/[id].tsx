...
import { 
  View, Image, Text, KeyboardAvoidingView, Platform, 
  TouchableWithoutFeedback, Keyboard, TouchableOpacity, TextInput 
} from 'react-native';
...
export default function EditProfile() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        setImage(userData.image ? { uri: `${StorageAPI}/${userData.image}` } : null);
      } catch (error: any) {
  ...
  const handleUpdateUser = async () => {
    try {
      ...
      // Append image hanya jika user pilih file baru
      if (image?.uri && !image.uri.startsWith(StorageAPI)) {
        const localUri = "file:///" + image.uri.split("file:/").join("");
      ...
      // Hanya append password jika user isi
      if (password.trim() !== "") {
      ...
      setTimeout(() => navigate.push('/home/profile'), 2000);
    } catch (error: any) {
      showError(error.response?.data?.message || "Terjadi kesalahan saat update user");
      console.error(error);
    }
  }
            ...
            <View className='pt-12 flex-col justify-center items-center px-6'>
                ...
                <Image 
                  source={
                    image?.uri 
                      ? { uri: image.uri } 
                      : user?.image 
                        ? { uri: `${StorageAPI}/${user.image}` } 
                        : guest
                  } 
                  className='w-[80px] h-[80px] rounded-full border-[1px] border-primary'
                  style={{ opacity: image?.uri && !image.uri.startsWith(StorageAPI) ? 0.6 : 1 }}
                />
              ...
              <View className='flex-col gap-2 w-full mt-6'>
                ...
                <TextInput 
                  ...
                  className='border-2 pl-4 font-poppins_regular text-[12px] rounded-lg border-gray w-full focus:border-primary' 
